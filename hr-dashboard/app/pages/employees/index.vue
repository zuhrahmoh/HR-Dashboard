    ...
    <div class="space-y-3">
      <label class="block">
        ...
        <div class="relative">
          ...
          >
            <path
              d="M10.5 3.75a6.75 6.75 0 1 0 0 13.5a6.75 6.75 0 0 0 0-13.5Z"
              stroke="currentColor"
              stroke-width="1.5"
            />
          ...
          <table class="min-w-full text-left text-sm">
            ...
            <tbody>
              ...
              >
                ...
                <td class="px-4 py-4 text-slate-200">{{ e.startDate ?? '—' }}</td>
              </tr>
              <tr v-if="filteredEmployees.length === 0" class="border-t border-slate-800">
                <td colspan="6" class="px-4 py-6 text-center text-slate-300">No matching employees.</td>
              </tr>
            </tbody>
          </table>
        </div>
...
const { data, pending, error } = await useFetch<Employee[]>('/api/employees')
...
function goToEmployee(employeeKey: string) {
  return navigateTo(`/employees/${employeeKey}`)
}
